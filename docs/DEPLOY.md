...
### 4. Scheduler Setup
The `vinsight-guardian` job needs a trigger to run periodic checks.
```bash
gcloud scheduler jobs create http guardian-scan \
  --schedule="0 */3 * * *" \
  --time-zone="America/New_York" \
  --uri="https://us-central1-run.googleapis.com/apis/run.googleapis.com/v1/namespaces/vinsight-ai/jobs/vinsight-guardian:run" \
  --http-method=POST \
